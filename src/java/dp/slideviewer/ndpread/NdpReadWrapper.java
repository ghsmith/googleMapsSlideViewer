...
public class NdpReadWrapper {
    ...
    private static final NdpRead ndpRead = (NdpRead) Native.loadLibrary("c:/windows/system32/NDPRead-x64", NdpRead.class);
    
    private static final long ERROR_RESPONSE_CODE = 0;

    public ImageInformation getImageInformation(String fileName) {
    ...
    public byte[] getImageSegment(String ndpiFileName, int xPositionOfDesiredCentreInNM,
            ...
            int desiredPixelWidth, int desiredPixelHeight) {
        ...
        if (resultCode == ERROR_RESPONSE_CODE)
        {
            ...
            throw new RuntimeException("Failed on second call to GetImageData with suitably sized buffer " + message);
        }
    ...
    private int determineRequiredBufferSize(String ndpiFileName, float desiredMagnification, NativeLong iPhysicalXPos,
        ...
        {
            String message = ndpRead.GetLastErrorMessage();
            throw new RuntimeException("Failed on initial call to GetImageData with zero sized buffer " + message);
        }
    ...
    private void setPhysicalCentre(String fileName, ImageInformation imageInformation) {
        ...
        {
            String message = ndpRead.GetLastErrorMessage();
            throw new RuntimeException("Failed to get image details through call to GetMap " + message);
        }
    ...
    private void setPixelWidthAndHeight(String fileName, ImageInformation imageInformation) {
        ...
        {
            String message = ndpRead.GetLastErrorMessage();
            throw new RuntimeException("Failed to get pixel size through call to GetSourcePixelSize " + message);
        }
